...
export function TaskBoardPanel() {
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      {/* Kanban Board */}
      <div className="flex-1 min-h-0 flex flex-col md:flex-row gap-4 p-4 overflow-x-auto overflow-y-auto md:overflow-y-hidden" role="region" aria-label={t('taskBoard')}>
        {statusColumns.map(column => (
          <div
            key={column.key}
            role="region"
            aria-label={t('columnAriaLabel', { title: column.title, count: tasksByStatus[column.key]?.length || 0 })}
            className="flex-1 md:min-w-80 min-h-0 bg-surface-0 border border-border/60 rounded-xl flex flex-col transition-colors duration-200 [&.drag-over]:border-primary/40 [&.drag-over]:bg-primary/[0.02]"
